...
three stores render Markdown in the description field.
    ...
    No data collection of any kind. No analytics, no telemetry, no accounts, and
    no server of ours anywhere. Nothing about your browsing is recorded or sent
    to us.

    By default the only network requests it makes are to Bluesky's public API,
    for the specific posts the page is already trying to show you.

    The one exception is Deep recovery, which is off unless you turn it on. To
    find replies that Bluesky drops from a thread entirely, it has to ask an
    external index — constellation.microcosm.blue, an independent public service
    not run by us or by Bluesky. When it is on, the address of the post you are
    viewing is sent to that service so it can name the replies pointing at it.
    Leave the switch off and nothing is ever sent there.
